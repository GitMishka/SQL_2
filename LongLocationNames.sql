 ...
 */
 ...
 SELECT Dense_Rank() OVER (
 	...
 	,CASE 
 		...
 		WHEN Propcode LIKE '%Lantern%'
 			THEN UPPER(RTRIM(code)) + ' - ' + replace(Propcode,', TN','') 
 		ELSE UPPER(RTRIM(code)) + ' - ' + replace(Propcode,', TN','') 
 		END AS propcode
 ...
 SELECT CASE 
 	...
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((Bugdet * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END Bugdet
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week1 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week1
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week2 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week2
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week3 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week3
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week4 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week4
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week5 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week5
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week6 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week6
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week7 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week7
 	,CASE 
 		WHEN [TOTAL Units] != 0
 			THEN cast((week8 * 100) / [TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END week8
 FROM #RegionCount
 WHERE region <> '% Occupied Total'
 ...
 SELECT CASE 
 	...
 	,CASE 
 		WHEN rc.[TOTAL Units] != 0
 			THEN cast((Bugdet * 100) / rc.[TOTAL Units] AS NUMERIC(18, 2))
 		ELSE '0'
 		END Bugdet
 	,CASE 
 		WHEN (
 				rc.[TOTAL Units] != 0
 				AND t.w1 != 0
 				) /*added condition t.w1 != 0' to avoid 
 divide by Zero error*/
 			THEN cast((week1 * 100) / t.w1 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week1
 	,CASE 
 		WHEN (
 			...
 			THEN cast((week2 * 100) / t.w2 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week2
 	,CASE 
 		WHEN (
 			...
 			THEN cast((week3 * 100) / t.w3 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week3
 	,CASE 
 		WHEN (
 			...
 			THEN cast((week4 * 100) / t.w4 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week4
 	,CASE 
 		WHEN (
 			...
 			THEN cast((week5 * 100) / t.w5 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week5
 	,CASE 
 		WHEN (
 			...
 			THEN cast((week6 * 100) / t.w6 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week6
 	,CASE 
 		WHEN (
 			...
 			THEN cast((week7 * 100) / t.w7 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week7
 	,CASE 
 		WHEN (
 			...
 			THEN cast((week8 * 100) / t.w8 AS NUMERIC(18, 2))
 		ELSE '0'
 		END week8
 FROM #RegionCount rc
 ...
 SELECT CASE 
 	...
 	,cast([TOTAL Units] AS VARCHAR) [TOTAL Units]
 	,'Units'
 	,cast(Bugdet AS NUMERIC(18, 2)) Bugdet
 	,cast(week1 AS NUMERIC(18, 2)) week1
 	,cast(week2 AS NUMERIC(18, 2)) week2
 	,cast(week3 AS NUMERIC(18, 2)) week3
 	,cast(week4 AS NUMERIC(18, 2)) week4
 ...
 SELECT CASE 
 	...
 	,CASE 
 		WHEN t.scode IS NOT NULL
 			AND @num <> 0
 			THEN 'Units (' + cast(t.totalunitdiff AS VARCHAR(100)) + ' available before ' + CONVERT(VARCHAR(10), t.minmovin, 101) + '( Property- ' + t.scode + ')' + ')'
 		ELSE 'Units'
 		END
 	,cast(Bugdet AS NUMERIC(18, 2)) Bugdet
 	,cast(week1 AS NUMERIC(18, 2)) week1
 ...
 SELECT CASE 
 		...
 		ELSE isequence + 1
 		END isequence
 	,cast((cast([TOTAL Units] AS NUMERIC(18, 2)) - cast(week1 AS NUMERIC(18, 2))) AS VARCHAR)
 ...
 ORDER BY isequence
 SELECT isequence,CONVERT(VARCHAR,propcode),CONVERT(VARCHAR,[Total units]),
 CONVERT(VARCHAR,Bugdet),CONVERT(VARCHAR,week1),CONVERT(VARCHAR,week2),CONVERT(VARCHAR,week3),
 CONVERT(VARCHAR,week4),CONVERT(VARCHAR,week5),CONVERT(VARCHAR,week6),CONVERT(VARCHAR,week7),CONVERT(VARCHAR,week8) FROM #Final
 UNION
 SELECT 0,'Location','Total Units','Budget',CONVERT(VARCHAR(10),@asEnddate,101),CONVERT(VARCHAR(10),DATEADD(DD,-7,@asEnddate),101),CONVERT(VARCHAR(10),DATEADD(DD,-14,@asEnddate),101),
