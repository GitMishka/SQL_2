...
declare @startDate date = @YTD_Begin
declare @endDate date = @YTD_End
...
*/
IF OBJECT_ID ('TempDb..#ytdsum') IS NOT NULL
DROP TABLE #ytdsum
create table #ytdsum
(Column_Index varchar(20)
,Cell_Data numeric)
...
left join #DateList dl on r.EvalMonth = dl.EvalMonth where evaldate between @YTD_Begin and @YTD_End



insert into #Clinical_KPI_Results_Staging 
select 'YTD' as Column_Index
,SUM(Cast(Cell_Data as int))
from #temp1
...
select distinct
'YTD'
,sum(Cell_Data) as Cell_Data
from (select distinct a2.evaldate, b2.Cell_Data from #DateList a2 join #temp_moveins b2 on a2.Column_Index = b2.Column_Index

where evaldate in (select max(dates) as [EvalDate] from @tmpTable as o
group by datepart(YEAR, dates), datepart(MONTH, dates))) rl
...
select distinct
'YTD'
,cast (sum(Cell_Data) as int) as Cell_Data
from (select distinct a2.evaldate, b2.Cell_Data from #DateList a2 join #temp_moveinssec b2 on a2.Column_Index = b2.Column_Index

where evaldate in (select max(dates) as [EvalDate] from @tmpTable as o
group by datepart(YEAR, dates), datepart(MONTH, dates))) rl

/*
select distinct a2.evaldate, b2.Cell_Data from #DateList a2 join #temp_moveinssec b2 on a2.Column_Index = b2.Column_Index
where EvalDate between @YTD_Begin and @YTD_End and where  
where evaldate in (select max(dates) as [EvalDate] from @tmpTable as o
...
(
	select	
    rl.EvalMonth	
  	,round(sum(cast(rl.Occupancy_Value as numeric(10,2))) / datediff(day, right(rl.EvalMonth,2) 
	+ '/01/' + left(rl.EvalMonth,4), dateadd(month, 1, right(rl.EvalMonth,2) + '/01/' + left(rl.EvalMonth,4))),0) as Cell_Data
...
select distinct
'YTD' as Column_Index
, cast(ceiling(round(avg(Cell_Data),1)) as int) as Cell_Data
from
(
	select	
    rl.EvalMonth	
  	,round(sum(cast(rl.Occupancy_Value as numeric(10,2))) / datediff(day, right(rl.EvalMonth,2) 
	+ '/01/' + left(rl.EvalMonth,4), dateadd(month, 1, right(rl.EvalMonth,2) + '/01/' + left(rl.EvalMonth,4))),0) as Cell_Data
	from #Resident_List rl where EvalDate between @YTD_Begin and @YTD_End
 	Group by
	rl.EvalMonth
) r
left join #DateList dl on r.EvalMonth = dl.EvalMonth 

/* Store average census value */
IF OBJECT_ID ('TempDb..#avgcensusvalue') IS NOT NULL
DROP TABLE #avgcensusvalue
create table #avgcensusvalue
(census numeric)
insert into #avgcensusvalue
select distinct
cast(ceiling(round(avg(Cell_Data),1)) as int) as Cell_Data
from
(
...
IF OBJECT_ID ('TempDb..#avg_age') IS NOT NULL
DROP TABLE #avg_age
...
select	
		'YTD',
  		cast(round(sum(cast(rl.Age as int)) / Count(rl.EvalMonth),0) as int) as Cell_Data
	from #Resident_List rl
	where rl.ResidentBirthDate is not null and right(left(rl.EvalMonth,4),2) = (select ( YEAR( GETDATE() ) % 100 ))
 	Group by
	rl.EvalMonth
insert into #Clinical_KPI_Results_Staging

select 'YTD', CAST(avg(cell_data) as  int) from #avg_age
...
IF OBJECT_ID ('TempDb..#temp_ytd_periods') IS NOT NULL
DROP TABLE #temp_ytd_periods

create table #temp_ytd_periods
(
Column_Index VARCHAR(100)
,Cell_Data NUMERIC(18, 2))

insert into #temp_ytd_periods
select distinct
dl.Column_Index
,(cast(r.Cell_Data as Int)) as Cell_Data
from
...
left join #DateList dl on r.EvalMonth = dl.EvalMonth where  dl.Evaldate between @YTD_Begin and @YTD_End
...
select distinct
'YTD'
,isnull(cast(sum(cell_data)as int),0) as Cell_Data
from #temp_ytd_periods
...
truncate table #Clinical_KPI_Results_Staging
truncate table #temp_ytd_periods
/*
...
,isnull(cast(sum(Cell_Data) as int),0) as Cell_Data
from  #temp_ytd_periods
/*(select distinct hMyPerson, MoveIn_Date, EvalMonth from #Resident_List_YTD 
where CareLevel = 'Premium' and MoveIn_Date between @YTD_Begin and @YTD_End) rl */
...
'YTD'
,sum(Cell_Data) as Cell_Data
from (select distinct a2.evaldate, b2.Cell_Data from #DateList a2 join #temp_moveouts b2 on a2.Column_Index = b2.Column_Index
where EvalDate between @YTD_Begin and @YTD_End and where  
where evaldate in (select max(dates) as [EvalDate] from @tmpTable as o
group by datepart(YEAR, dates), datepart(MONTH, dates))) rl
...
'YTD'
,cast(sum(Cell_Data) as int) as Cell_Data
from (select distinct a2.evaldate, b2.Cell_Data from #DateList a2 join #temp_moveoutssec b2 on a2.Column_Index = b2.Column_Index
...
select distinct 
'YTD' as Column_Index,
COALESCE(cast(r.Cell_Data as float),0) as Cell_Data
from
(
select	rl.EvalMonth ,cast(Count(rl.EvalMonth) as float) as Cell_Data
	from #Discharge_List rl
	where MoveOut_Category not in ( 'Death' ) and LOS_Days < 90 
...
left join #DateList dl on r.EvalMonth = dl.EvalMonth where EvalDate between @YTD_Begin and @YTD_End
if @@ROWCOUNT = 0
insert into #temp1 values('YTD', 0)
...
select distinct 
Column_Index
,isnull(sum(cast((Cell_Data) as int)),0) as Cell_Data
from #temp1
Group by Column_Index
...
select distinct
'YTD'
,CAST(sum(cell_data) as int) / (select count(distinct(right(left(EvalMonth,6),4))) from #DateList where right(left(EvalMonth,4),2) = (select ( YEAR( GETDATE() ) % 100 )))
/*,cast(round(sum(cast(LOS_Months as numeric(10,2))) / Count(EvalMonth),0) as int) */
as Cell_Data
from #temp_ytd_periods
...
select distinct
'YTD'
,sum(rl.los_days) as Cell_Data
from (select distinct thmy, los_days, MoveInDate, EvalMonth from #Discharge_List_YTD where MoveInDate between @YTD_Begin and @YTD_End) rl
...
(select	rl.EvalMonth ,Count(rl.EvalMonth) as Cell_Data
	from #Discharge_List rl 
	where MoveOut_Category = 'Higher LOC' and carelevel in ('Personal Care','Assisted Living','PC Bundled')
 	Group by
	rl.EvalMonth
) r
...
select distinct
'YTD'
,cast(Count(EvalMonth) as numeric(10,2)) /  (select distinct
cast(round(avg(Cell_Data),1) as decimal (12,2)) as Cell_Data
from
(
	select	
    rl.EvalMonth	
...
left join #DateList dl on r.EvalMonth = dl.EvalMonth ) as Cell_Data
from #Incident_List_YTD
where Incident_Type = 'FAL'
...
select distinct
'YTD'
,cast(cast(cast(Count(EvalMonth) as numeric(10,2)) /  (select distinct
cast(round(avg(Cell_Data),1) as decimal (12,2)) as Cell_Data
from
(
	select	
...
left join #DateList dl on r.EvalMonth = dl.EvalMonth ) * 100 as numeric(10,2)) as varchar) + '%' as Cell_Data 
from #Incident_List_YTD
where Incident_Type = 'FAL' 
*/
truncate table #temp1
...
/* add data from month to date in this year, then divide it by amount of month in this year for current date range to find average for the year, then divide by average census.
use different value conversions for math, at last conversion turn it into string to add '%'. = YTD Calculation
*/
insert into #Clinical_KPI_Results_Staging
/*insert into #ytdsum */
select distinct 
/*#temp2.Column_Index, */ 
'YTD' ,
CAST(CEILING(CAST(((sum(ROUND(cast(#temp2.Cell_Data as float) / cast(#temp1.Cell_Data as float),2))*100) / (select count(distinct(right(left(EvalMonth,6),4))) 
from #DateList where right(left(EvalMonth,4),2) = (select ( YEAR( GETDATE() ) % 100 )))) / (select census from #avgcensusvalue) * 100 as float)) as VARCHAR) + '%'
as Cell_Data from #temp2
...
select distinct
'YTD',
cast(isnull(cast((Cell_Data) / (select census from #avgcensusvalue) * 100 as int),0) as varchar) +'%' as Cell_Data
from #ytdsum 
*/
...
IF OBJECT_ID ('TempDb..#tempunique1') IS NOT NULL
DROP TABLE #tempunique1

Create Table #tempunique1
(
Column_Index varchar(25)
,Cell_Data varchar(100)
)
IF OBJECT_ID ('TempDb..#tempunique2') IS NOT NULL
DROP TABLE #tempunique2

Create Table #tempunique2
(
Column_Index varchar(25)
,Cell_Data varchar(100)
)
insert into #tempunique1
select distinct
dl.Column_Index
,cast(r.Cell_Data as Int) as Cell_Data1
from
...
insert into #Clinical_KPI_Results_Staging
select distinct 'YTD' as Column_Index, 
cast(ceiling(cast(((((sum((ROUND(cast(#tempunique2.Cell_Data as float) 
/ cast(#tempunique1.Cell_Data as float),2)))*100 ) / (select count(distinct(right(left(EvalMonth,6),4))) 
from #DateList where right(left(EvalMonth,4),2) = (select ( YEAR( GETDATE() ) % 100 ))))
/ (select census from #avgcensusvalue)) * 100) as float)) as varchar) + '%' 
as Cell_Data from #tempunique2

join #tempunique1 
	on #tempunique2.Column_Index = #tempunique1.Column_Index

/*
insert into #Clinical_KPI_Results_Staging
select distinct 'YTD' as Column_Index, 
	cast((sum((ROUND(cast(#tempunique2.Cell_Data as float) 
	/ cast(#tempunique1.Cell_Data as float),2)))*100 /  
	(select census from #avgcensusvalue)) * 100 as numeric(10,2)) as Cell_Data from #tempunique2
join #tempunique1 
	on #tempunique2.Column_Index = #tempunique1.Column_Index
*/
...
select distinct
'YTD',
isnull(sum(Cell_Data),0) as Cell_Data
from #ytdsum
...
insert into #Clinical_KPI_Results_Staging
select distinct 'YTD' as Column_Index, 
cast(cast(ceiling(((((sum((ROUND(cast(#temp2.Cell_Data as float) 
/ cast(#temp1.Cell_Data as float),2)))*100 ) / (select count(distinct(right(left(EvalMonth,6),4))) 
from #DateList where right(left(EvalMonth,4),2) = (select ( YEAR( GETDATE() ) % 100 ))))
/ (select census from #avgcensusvalue)) * 100)) as float) as varchar) + '%' 
as Cell_Data from #temp2
...
insert into #ytdsum
select distinct #temp2.Column_Index, 
	cast(((ROUND(cast(#temp2.Cell_Data as float) 
	/ cast(#temp1.Cell_Data as float),2))*100)
	/ (select count(distinct(right(left(EvalMonth,6),4))) 
from #DateList where right(left(EvalMonth,4),2) = (select ( YEAR( GETDATE() ) % 100 )))
	/ (select census from #avgcensusvalue) as int)
...
select distinct
'YTD',
cast(isnull(cast(sum(Cell_Data) / (select census from #avgcensusvalue) * 100 as int),0) as varchar) +'%' as Cell_Data
from #ytdsum
*/
...
update #Clinical_KPI_Results set Report_Index = 'S1055' where Description = 'Total New Move Ins + Secondary' 
...
update #Clinical_KPI_Results set Report_Index = 'S2013' where Description = 'Total Discharges + Secondary'
