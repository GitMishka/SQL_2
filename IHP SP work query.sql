...
--DECLARE 
	...
	--SET @BOMActual  = @BOM  
	--SET @BOM = '1-01-2022'--DATEADD(dd, -1, @BOMActual)    
	--set @EOM = '3-28-2022'
	--SET @IncludeMoveOutDate = CASE WHEN @flag IN ('1','2','3') THEN 'No' ELSE 'YES' END
	--set @flag = 4	
	--set @ShowSeccondResident = 'No'
	--SET @AdditionalUnit = 'No'
	--SET @CareLevel = ''
	--	SELECT @CareLevel = @CareLevel + CONVERT(VARCHAR(100), l1.hmy) + ','
	--	FROM listoption l1
	--	WHERE l1.listname = 'CareLevel' 
	--SET @CareLevel = LEFT( @CareLevel, Len( @CareLevel ) - 1 )

declare @BOM  datetime
declare @EOM datetime,
@BOMActual datetime,
@hprop VARCHAR(4000),
@CareLevel VARCHAR(4000),
@flag   VARCHAR(1),
@ShowSeccondResident VARCHAR(3),
@IncludeMoveOutDate VARCHAR(3),
@AdditionalUnit  VARCHAR(3)
set @bom = DATEADD(dd, -1, @BOMActual)
SET @BOMActual  = '1-1-2022'
SET @EOM = '3-28-2022'
SET @ShowSeccondResident = 'No'
SET @AdditionalUnit  = 'No'
...
SELECT @hprop = @hprop + CONVERT(VARCHAR(100), p.hmy) + ',' 
FROM Property P
WHERE
1 = 1

SET @hprop = LEFT( @hprop, Len( @hprop ) - 1 )
