...
SELECT 
	...
	, case 
		when t.istatus in (2,8) then 'R' /* GREEN - future,whitelisted*/
		when t.istatus = 4 then 'N' /* RED - prospect*/
		when t.istatus = 11 then 'L' /* ORANGE - on leave*/
		when t.istatus = 0 and t.dtmovein > @Eval_Date then 'C' /* BLUE - current*/
		when t.istatus = 1 and srhs.dtBillingEnd > @Eval_Date and srhs.dtBillingEnd > t.DTMOVEOUT then 'F' 
		else 'O' /* KHAKI */
	end as Unit_Status
	,ltrim(rtrim(u.scode)) as Unit
	,ltrim(rtrim(t.sLastName))+', '+ltrim(rtrim(t.sFirstName)) + ' Billing End: ' +isnull(cast(cast(dtBillingEnd as date) as varchar),'') AS Resident_Name /* +' ('+ ltrim(rtrim(t.scode))+')' AS Resident_Name */
...
inner join SeniorResidentHistoryStatus srhs on t.HMYPERSON = srhs.hResident
...
select * from #Inventory_Occupancy_Residents where unit_status = 'F'
...
,case 
	...
	when r.Unit_Status = 'F' then '(FM) ' + r.Resident_Name 
...
where 
unit_status = 'F'
--resident_name = 'Cephus, Eloise'
--community like '%Chattanooga Shallowford%' and unit = '301'
--resident_name = 'Baumgartner, Ruth'
--community like '%Spring Hill%' and unit = '225'
*/
