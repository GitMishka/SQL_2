...
SELECT 
	...
	, case 
		...
		when t.istatus = 0 and t.dtmovein > @Eval_Date and t.dtmovein <= t.DTSIGNDATE then 'C' 
		else 'O' 
	end as Unit_Status
	...
	,ts.status ResStatus
	,srhs.sPrivacyLevelCode


from  #DateList d
left join property P  on 1=1
INNER JOIN Tenant T ON T.Hproperty = P.hmy
inner JOIN SeniorResidentHistoryStatus srhs on (srhs.hResident = t.hmyperson)
...
/*
select * from #Resident_List_YTD
select * from #Resident_List
where unit_status <> 'O'
select distinct hMyPerson, MoveIn_Date, EvalMonth, Unit_Status, community from #Resident_List
where unit_status <> 'O'
select * from #Resident_List
*/
--select * from #Resident_List_YTD
...
'YTD'
,Count(rl.EvalMonth) as Cell_Data
from (select distinct hMyPerson, MoveIn_Date, EvalMonth, Unit_Status from #Resident_List_YTD where Unit_Status = 'C' and MoveIn_Date between @YTD_Begin and @YTD_End) rl
...
insert into #Clinical_KPI_Results_Staging select 'Description', 'Total Financial Move Ins'
...
(
	...
	rl.EvalMonth	
  	,Count(rl.EvalMonth) as Cell_Data
	from (select distinct hMyPerson, MoveIn_Date, EvalMonth, CareLevel, EvalDate,sPrivacyLevelCode from #Resident_List) rl
 	WHERE 
		rl.sPrivacyLevelCode = 'SEC'
		and LEFT(CONVERT(varchar, rl.MoveIn_Date,112),6) = rl.EvalMonth /* Move In in Eval Month */
		and rl.EvalDate = convert(date,(DATEADD (dd, -1, DATEADD(mm, DATEDIFF(mm, 0, right(rl.EvalMonth,2) + '/01/' + left(rl.EvalMonth,4)) + 1, 0))))
	Group by
	rl.EvalMonth
...
'YTD'
,Count(rl.EvalMonth) as Cell_Data
from (select distinct hMyPerson, MoveIn_Date, EvalMonth from #Resident_List_YTD where sPrivacyLevelCode = 'SEC' and MoveIn_Date between @YTD_Begin and @YTD_End) rl
...
insert into #Clinical_KPI_Results_Staging select 'Report_Index', 'S0000'
insert into #Clinical_KPI_Results_Staging select 'Description', 'Total Second Residents'
...
from  #DateList_YTD d
	...
	Inner Join SeniorResidentStatus srs on (srs.iStatus = t.iStatus) -- select * from SeniorResidentHistoryStatus where sprivacylevelcode = 'SEC'
	inner join seniorresident sr on t.hmyperson = sr.residentid 
	INNER JOIN SeniorResidentHistoryStatus  Srh ON Srh.hResident  = sr.ResidentId 
	ANd srh.hmy in (SELECT MAX(srh1.hmy)
      	From SeniorResidentHistoryStatus srh1 
...
--update #Clinical_KPI_Results set Report_Index = 'S4120' where Description = 'Financial Move Ins'
--update #Clinical_KPI_Results set Report_Index = 'S4130' where Description = 'Secondary Residents'
update #Clinical_KPI_Results set Report_Index = 'S4120' where Description = 'Total Financial Move Ins'
update #Clinical_KPI_Results set Report_Index = 'S4130' where Description = 'Total Second Residents'
