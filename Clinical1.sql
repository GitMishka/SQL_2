...
Declare @MonthsLookBack int = 6 
DECLARE @PropertyCode VARCHAR(4000), @DpDate DATETIME
SET @PropertyCode = ''

/* 6 for Normal Execution of Report */
...
insert into @EvalProperties Select 17, 'all'
/*
...
WHERE l.hProplist IN (
		select hProp FROM @EvalProperties
		)
...
DECLARE @carelev VARCHAR(MAX)
	,@ResStatus VARCHAR(MAX)
	,@ContTyp VARCHAR(MAX)
	,@OccType VARCHAR(20)

SET @OccType = 'Financial'
SET @ContTyp = ''
SET @carelev = ''
SET @ResStatus = ''
...
FROM listoption P
WHERE ListName = 'CareLevel'

SELECT @ResStatus = @ResStatus + LTRIM(RTRIM(iStatus)) + ','
FROM SeniorResidentStatus


SELECT @ContTyp = @ContTyp + LTRIM(RTRIM(l1.listoptioncode)) + ','
FROM listoption l1
WHERE l1.listname = 'ContractType'
...
/* Create Financial Move In Tables */

IF OBJECT_ID ('TempDb..#FinancialMoveIn') IS NOT NULL
DROP TABLE #FinancialMoveIn

CREATE TABLE #FinancialMoveIn (
...
INSERT INTO #FinancialMoveIn
EXEC SeniorMoveInMoveOutDetailFinancial 'MoveIn'
	,@PropertyCode
	,''
	,'Actual'
	,''
...
IF OBJECT_ID ('TempDb..#MoveInsFinal') IS NOT NULL
DROP TABLE #MoveInsFinal

SELECT phMy,PropName,MONTH(moveindate) Rep_Month, YEAR(moveindate) Rep_Year,
SUM(case when d.privacylevel in ('TAS','TBS','TCS','TOA','TOB','TOC') then 0.33 
when d.privacylevel in ('SPA','SPB','DAS','DBS') then 0.50 
when d.privacylevel in ('QAS','QBS','QCS','QDA','QDB','QDC','QDD','QDS') then 0.25 else 1 end) cnt
INTO #MoveInsFinal
from #FinancialMoveIn d
GROUP BY phMy,PropName,MONTH(moveindate), YEAR(moveindate)
...
/*Financial Move In*/
UPDATE #Clinical_KPI_Results SET Period_1 = (
SELECT MICnt FROM (
select Rep_Month,Rep_Year,SUM(Cnt) MICnt  
from #MoveInsFinal
WHERE Rep_Month = MONTH(DATEADD(MM,-5,@YTD_End))
AND Rep_Year = YEAR(DATEADD(MM,-5,@YTD_End))
GROUP BY Rep_Month,Rep_Year
) A
)
WHERE Description = 'Total New Move Ins'

UPDATE #Clinical_KPI_Results SET Period_2 = (
SELECT MICnt FROM (
select Rep_Month,Rep_Year,SUM(Cnt) MICnt  
from #MoveInsFinal
WHERE Rep_Month = MONTH(DATEADD(MM,-4,@YTD_End))
AND Rep_Year = YEAR(DATEADD(MM,-4,@YTD_End))
GROUP BY Rep_Month,Rep_Year
) A
)
WHERE Description = 'Total New Move Ins'

UPDATE #Clinical_KPI_Results SET Period_3 = (
SELECT MICnt FROM (
select Rep_Month,Rep_Year,SUM(Cnt) MICnt  
from #MoveInsFinal
WHERE Rep_Month = MONTH(DATEADD(MM,-3,@YTD_End))
AND Rep_Year = YEAR(DATEADD(MM,-3,@YTD_End))
GROUP BY Rep_Month,Rep_Year
) A
)
WHERE Description = 'Total New Move Ins'

UPDATE #Clinical_KPI_Results SET Period_4 = (
SELECT MICnt FROM (
select Rep_Month,Rep_Year,SUM(Cnt) MICnt  
from #MoveInsFinal
WHERE Rep_Month = MONTH(DATEADD(MM,-2,@YTD_End))
AND Rep_Year = YEAR(DATEADD(MM,-2,@YTD_End))
GROUP BY Rep_Month,Rep_Year
) A
)
WHERE Description = 'Total New Move Ins'

UPDATE #Clinical_KPI_Results SET Period_5 = (
SELECT MICnt FROM (
select Rep_Month,Rep_Year,SUM(Cnt) MICnt  
from #MoveInsFinal
WHERE Rep_Month = MONTH(DATEADD(MM,-1,@YTD_End))
AND Rep_Year = YEAR(DATEADD(MM,-1,@YTD_End))
GROUP BY Rep_Month,Rep_Year
) A
)
WHERE Description = 'Total New Move Ins'

UPDATE #Clinical_KPI_Results SET Period_6 = (
SELECT MICnt FROM (
select Rep_Month,Rep_Year,SUM(Cnt) MICnt  
from #MoveInsFinal
WHERE Rep_Month = MONTH(@YTD_End)
AND Rep_Year = YEAR(@YTD_End)
GROUP BY Rep_Month,Rep_Year
) A
)
WHERE Description = 'Total New Move Ins'

UPDATE #Clinical_KPI_Results SET Period_5v6 = (CONVERT(NUMERIC(18,2), Period_5) - CONVERT(NUMERIC(18,2), Period_6)) WHERE Description = 'Total New Move Ins'
...
WHERE Description = 'Total Discharges'
...
UPDATE #Clinical_KPI_Results SET Period_5v6 = (CONVERT(NUMERIC(18,2), Period_5) - CONVERT(NUMERIC(18,2), Period_6)) WHERE Description = 'Total Discharges'
...
select * from #MoveInsFinal where phMy=20
