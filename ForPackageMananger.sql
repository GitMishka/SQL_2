 ...
 SELECT Dense_Rank() OVER (
 	...
 	,CASE 
 		...
 		WHEN Propcode LIKE '%Lantern%'
 			THEN replace(replace(Propcode, 'The Lantern at Morning Pointe of', ''),',','') + ' ' + 'Lantern' + ' ' + ' (' + RTRIM(code) + ')'
 		ELSE replace(replace(Propcode, 'Morning Pointe of', ''),',','') + ' (' + RTRIM(code) + ')'
 		END AS propcode
 ...
 SELECT CASE 
 	...
 	,CASE 
 		WHEN t.scode IS NOT NULL
 			AND @num <> 0
 			THEN 'Units (' + cast(t.totalunitdiff AS VARCHAR(30)) + ' available before ' + CONVERT(VARCHAR(10), t.minmovin, 101) + '( Property- ' + t.scode + ')' + ')'
 		ELSE 'Units'
 		END
 	,cast(Bugdet AS NUMERIC(18, 2)) Bugdet
 	,cast(week1 AS NUMERIC(18, 2)) week1
