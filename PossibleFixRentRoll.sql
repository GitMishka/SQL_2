...
SELECT  p.hmy,
		...
		,
        --isnull(myview.PrivacyLevelCode, (case when su.UnitCapacityCount =1 then 'P'
        --                                when su.UnitCapacityCount =2 then 'S'
        --                                when su.UnitCapacityCount =3 then 'T'
        --                                when su.UnitCapacityCount =3 then 'Q' end )),
		PrivacyLevelCode,
...
(
        ...
        INNER JOIN  SeniorUnitRentHistory surh on surh.UnitHistoryId = suh.UnitHistoryId AND surh.PrivacyLevelCode in ('PRI' , 'SEC','SPB','SPA')
        WHERE 1=1
        ...
        (
                ...
                (
						...
						join SeniorUnitRentHistory surh on suh2.UnitHistoryID = suh2.UnitHistoryID
        ...
        --AND LEFT(surh.privacylevelcode,1) in
        --(
        --        SELECT 'P' WHERE suh.unitcapacitycount in (1,2,3,4)
        --        UNION
        --        SELECT 'S' WHERE suh.unitcapacitycount in (2,3,4)
        --        UNION
        --        SELECT 'T' WHERE suh.unitcapacitycount in (3,4)
        --        UNION
        --        SELECT 'Q' WHERE suh.unitcapacitycount in (4)
        --)
...
FROM #Property p
        ...
        INNER JOIN SeniorResident sr ON sr.ResidentID = t.hmyPerson and us.PrivacyLevel = sr.PrivacyLevelCode --select * from #Units us join SeniorResident sr on us.PrivacyLevel = sr.PrivacyLevelCode
