...
from 
	...
	left join (
                ...
                from 
                  ...
                  where 
                    ph.dtCompleted between @wk_startdate and @wk_enddate
                    and pa.ActivityCategory in ('TOU','INI') and pa.bUseForReporting = 1 and pa.oldtypecode = 'AT2'
                  ) sp on p.hMy = sp.hMy
...
from 
	...
	left join (
		    ...
		    left join Property p on p.hmy = ph.hproperty
                    ...
                    where 
                    ph.dtCompleted between @lm_startdate and @lm_enddate
                    and pa.ActivityCategory not in ('NRF','RFF','STT','MSC') /*Put in MSC to filter out unwanted activities 5-10-2022*/
					and sp.sStatus <> 'Referral'      
                  ) sp on p.hMy = sp.hMy
	...
	left join (
		...
		from 
			...
			left join SeniorProspect sp on sp.hMy = ph.hProspect
                ...
                where 
                ph.dtCompleted between @wk_startdate and @wk_enddate
              and pa.ActivityCategory not in ('NRF') and pa.ActivityName  in ('Marketing Visit')
			  /* and pa.ActivityCategory in ('ERA') and pa.ActivityName not in ('SalesMail')  CHANGED: Filtered out SalesMail 2022-05-12 */
				and sp.sStatus = 'Referral'      
                  ) sp on p.hMy = sp.hMy
